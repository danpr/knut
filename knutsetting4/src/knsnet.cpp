...
KNSNet::KNSNet (const QString upsAddress, const QString upsName, const unsigned short port, unsigned int countRepeat, const unsigned  int delay)
...
void KNSNet::slotDisconnected (void) {
...
void KNSNet::close (void) {
  ...
  if ((m_state == Connected) || (m_state == Connecting)) {
    ...
    if (m_commandSocket->bytesToWrite() > 0) KNSSleep::msleep(1000);
...
void KNSNet::newUPS (const QString upsAddress, const QString upsName, const unsigned short port) {
...
//int KNSNet::getState ( void ){ return m_state; }

bool KNSNet::isDescription ( void ){ return m_description; }

int KNSNet::getNutProtocol ( void ) { return m_nutProtocol; }

KNSNet::nutVarVersion KNSNet::getNutVariables ( void ) {
...
bool KNSNet::switchedToTCP ( void ) { return m_switchToTCP; }


KNSNet::NetError KNSNet::getUpsVars ( void) {
  ...
  if (m_state == Connected) {
    ...
    do {
      ...
      if ( readStatus() != KNSNet::WAIT) return returnValue;
      countWaitRevision--;
      if (countWaitRevision) KNSSleep::msleep(2);
      }
...
KNSNet::NetError KNSNet::getUpsValues (const bool allVars ) {// allVars = true vse; allVars = false jen activate
...
int KNSNet::getDescription (bool always) {
...
int KNSNet::instantCommand (const QString command, const QString userName, const QString password, const bool onlyCommand) {
  m_errorState=NoError; // vynulujeme chyby
...
int KNSNet::setVariable (const QString variable, const QString value, const QString userName, const QString password, const bool onlyVariable) {
  m_errorState=NoError; // vynulujeme chyby
  if (m_state == Connected) {
    QString outBuffer;
    QString setCommand;
...
int KNSNet::readNumberVars (typeOfVar typVar) {
...
int KNSNet::readNumberComms (void) {
//  Vraci pocet prikazu
  m_errorState=NoError;
...
int KNSNet::readIComm (const int seqNumber, upsICommDef& upsIComm) {
  m_errorState =NoError;
  if (m_state == Connected) {
...
int KNSNet::readVars (const QString name, struct upsVarDef& allVar) {
  m_errorState =NoError;
  if (m_state == Connected) {
  ...
  int KNSNet::readVars (const int seqNumber, struct upsVarDef& allVar, const typeOfVar typVar) {
...
QString KNSNet::readStringVar (const QString name) {
...
QString KNSNet::readEnumValueVar (const QString name, const int valueNumber) {
...
int KNSNet::existName ( const QString name ) {
...
int KNSNet::version (int countRepeat, const int delay) {
  ...
  do {
    ...
    countRepeat--;
    if (countRepeat) KNSSleep::msleep (delay);
    }
  while ((countRepeat));
  return m_errorState;
  }
...
KNSNet::NetError KNSNet::getFirstUpsName (QString& firstUpsName) {
  QByteArray  inBuffer;
  QString  outBuffer;
...
int KNSNet::setKey (const QString line) {
...
QString KNSNet::parseValue (const QString line) {
...
int KNSNet::parseTypeValue (QString line, int& lenString ) {
  int ret=0;
  QString word;
...
int KNSNet::parseLine(QString& line, QString& upsName ,QString& varName, QString& value, int& typeValue, int& lenString) {
  int posChar;
...
KNSNet::NetError KNSNet::newDescription (const QByteArray  inBuffer, QString& upsDescription) {
  QString  outBuffer;
...
void KNSNet::cleanConnecting (void) {
  qint64 rMaxLen; 
...
KNSNet::NetError KNSNet::getUpsData (const QByteArray sBuffer, QString& rBuffer, const QByteArray endString) {
...
//finds right end of input data
 ...
 void KNSNet::setRWVars (const QString varName, const bool valueType, const int varMax, QVector<QString>* enumValues) {
...
void KNSNet::upsSetType (struct upsVarDef& upsVar, const QString name, const QString value) {
...
void KNSNet::genStatusFlags (QString value) {
  m_upsStatusVar = 0;
...
int KNSNet::getDescription1 (bool always) {
...
int KNSNet::getDescription2 (bool always) {
...
KNSNet::NetError KNSNet::getUpsVars1 ( void) {
...
KNSNet::NetError KNSNet::getUpsVars2 ( void) {
  QByteArray  inBuffer;
  QString  outBuffer;
  QString  varOutBuffer;
  upsVarDef upsVar;
...
KNSNet::NetError KNSNet::getUpsValues1 (const bool allVars ) {// allVars = true vse; allVars = false jen activate
    QString endChar;
    QByteArray  inBuffer;
    QString  outBuffer;
    QVector<upsVarDef>::iterator it;
...
KNSNet::NetError KNSNet::getUpsValues2 (const bool allVars ) {// allVars = true vse; allVars = false jen activate
  QString varName;
  QString upsNameRet;
  QString value;
  int typeValue;
...
KNSNet::NetError KNSNet::sendComm (const QString command, const QString arg1, const QString arg2, const bool useUpsName) {
  NetError localError;
...
int KNSNet::setActiveUpsVar ( const QString name, const bool active ) {
  m_errorState =NoError;
  if (m_state == Connected) {
    QVector<upsVarDef>::iterator it;
    for (it = m_listVars.begin(); it != m_listVars.end(); it++) {
...
int KNSNet::setActiveAllUpsVars ( const bool active  ) {
...
KNSNet::NetError KNSNet::upsTranslateError (const QString string) {
...
KNSNet::NetError KNSNet::upsOldTranslateError (const QString string) {
...
void KNSNet::slotConnectionError(QAbstractSocket::SocketError socketError) {
...
void KNSNet::slotConnected(void) {
  int n;
    ...
    if ((n = version (m_countRepeat, m_delay)) == 0) { // 0 OK, 0> error
      ...
      // if upsName is empty reads first ups name from upsd server 
        if ((m_errorState =KNSNet::getFirstUpsName (m_upsName)) != NoError) { // reading first upsName
...
void KNSNet::slotHostFound(void) {
...
void KNSNet::slotTimerReconnect(void) {
